...
public final class Launcher extends Activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        } else registerReceiver(mCloseSystemDialogsReceiver, filter);
...
//            final Intent intent = ((ShortcutInfo) tag).intent;
            ...
            if(LauncherUtils.isSystemApplication(
                    this,
                    ((ShortcutInfo) tag).intent.getComponent().getPackageName()
            ))
            {
                ((ShortcutInfo) tag).intent.setSourceBounds(new Rect(pos[0], pos[1],
                        pos[0] + v.getWidth(), pos[1] + v.getHeight()));

                boolean success = startActivitySafely(v, ((ShortcutInfo) tag).intent, tag);
            }
            else
            {
                try{
                    ...
                    if(reserveIntent != null) startActivity(reserveIntent);
                }catch (Exception e)
                {
                }
            }
